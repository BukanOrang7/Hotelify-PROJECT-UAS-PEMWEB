...
<form method="POST" action="{{ route('admin.blog.update', $post) }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-sm max-w-2xl">
    @csrf
    @method('PUT')
    ...
    <div class="mb-6">
        ...
        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-500 transition"
            ...
            @if($post->featured_image)
                <img id="preview" src="{{ asset('storage/' . $post->featured_image) }}" class="mx-auto max-h-48 mt-4">
            @else
